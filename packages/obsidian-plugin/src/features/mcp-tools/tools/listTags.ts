...
export const listTagsSchema = type({
  ...
  "Lists all tags used across the vault with their usage counts, aggregating both inline `#tags` and frontmatter tags. A tag used only inside a folder the vault owner has hidden is absent from this list entirely, rather than shown with a lower count.",
);
