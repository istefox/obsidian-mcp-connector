...
// The version line makes every release's main.js byte-unique. Identical
// bundles across releases share one digest, GitHub then piles every tag's
// attestation onto it, and Obsidian's review rejects the digest because the
// attestations it picks no longer match the release under review. Version
// comes from package.json (not GITHUB_REF_NAME) so Obsidian's rebuild-from-
// source verification stays byte-for-byte reproducible at the tag.
const banner = `/*
THIS IS A GENERATED/BUNDLED FILE BY BUN
obsidian-mcp-connector v${version}
