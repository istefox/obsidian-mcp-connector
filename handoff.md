...
> **Aggiornato 2026-05-09 mattina (sessione "risolviamo le pending una a una" + deep audit di chiusura): folotp #54 cross-tracker ack — ruleset General narrow `feat/**` → `feat/http-embedded` esatto + cleanup orphan branch `feat/0.4-migration-ux-cleanup` — FR #88 SHIPPED (PR #92 squash `e01617f`, Option A errno-keyed, mock realism + 2 regression-locked test, plugin suite 747→749) — issue #88 chiusa manualmente — passive #4 marcoaperez + #5 store #11919 confermati quiescenti. Post-session cleanup: `fix/73-templates-execute-compat-shim` deleted (8 giorni stale, 3 commit shipped via PR #75 squash). **Deep audit 16 check su 4 batch ha rivelato 1 finding critico: CHANGELOG.md `[Unreleased]` block era VUOTO** — i 3 PR (#90 #91 #92) avevano saltato l'entry-add step. Backfilled in commit `c5d9f02` con 5 entry (2 Added + 2 Fixed + 1 Changed) ready per `[Unreleased] → [0.4.6]` promote al prossimo cut. **HEAD `feat/http-embedded` = `c5d9f02`** (9 commit avanti vs #87 baseline pre-sessione 2026-05-08). Final remote: solo `main` + `feat/http-embedded`, zero orphan. 4 finding medium flagged per tracciamento futuro (CLAUDE.md "Current versions" stale 13gg, README "19 tools" stale → 25, markdown-patch#11 folotp informativo non actionable, 19 dangling commits GC-eligible).**
>
...
- **`CLAUDE.md` "Current versions (2026-04-26)"** stale 13 giorni: dichiara `main` ships `0.3.10` (real `0.3.12`), `feat/http-embedded` ships `0.4.0-alpha.4` (real `0.4.5+9 commit`). Sezione "Project status (2026-04-26)" + "Pending work" similarmente datate. Decisione: refresh ora (~15 min) o accetta drift fino a major event (es. 0.4.6 cut o store accept).
