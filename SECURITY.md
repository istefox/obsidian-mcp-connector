...
### Content (folder exclusion, OMC-040)

Every tool and prompt handler receives its `App` from a single guarded facade (`shared/guardedApp.ts`), composed once at startup over `mcpTools.excludedFolders: string[]` (Settings → MCP Connector → Hidden folders). A file inside an excluded folder is unreachable through **every** MCP-facing surface — vault reads, writes, listings, the link graph, tag counts, prompt discovery and transclusion — and behaves exactly as if it did not exist: the same `not_found`-shaped response a genuinely absent path would produce, never a distinguishable "access denied" (a denial that differed from not-found would itself confirm the folder exists). Four tools that reach vault content through a path the facade cannot follow — `execute_obsidian_command`, `execute_dataview_query`, `execute_template`, `search_vault` — are disabled outright while any folder is hidden. (`search_vault` was missing from this set until 2026-08-24: its `dataview` query mode shares `execute_dataview_query`'s route around the facade, and it wasn't caught until Discussion #493 reported that a query against a hidden folder still returned results.)

**This is a guardrail against an AI client wandering into the wrong folder, not a security boundary against someone who wants your files.** It does not change the threat model below in three ways an attacker or a careless reviewer could otherwise assume it does:

- **It is not encryption or redaction.** The files are untouched on disk; any other plugin, sync client, backup tool, or a compromised Obsidian process still reads them (see "Compromised Obsidian" below).
- **It does not retroactively protect content already sent.** Anything the model already read before a folder was excluded stays wherever that conversation was sent.
- **An older plugin build ignores the setting.** `excludedFolders` is preserved across a downgrade (every settings recipe spreads the existing slice) but a pre-OMC-040 build does not enforce it — this is not detectable or fixable from inside the plugin.

Matching is exact and case-sensitive by design (ADR-0020 §D13): case-insensitive matching would be unsound on a case-sensitive vault, where `Journal/` and `journal/` are different folders. The settings UI's stale-entry marker exists specifically to catch a hand-typed folder name that matches nothing.

Full design and the complete list of consequences: [ADR-0020](docs/architecture/ADR-0020-omc-040-vault-folder-exclusion.md).
...
- **Content-side filtering or redaction of vault data passed to the model.** When the AI client requests a note via `get_vault_file` or a search via `search_vault_smart`, the plugin returns vault content as-is. The plugin does not scan for PII, secrets, credentials, or other sensitive content before returning it. If your vault contains content you would not paste into the model's chat window, do not connect it to an MCP client without your own filtering layer in front. **Amended (OMC-040):** the folder-exclusion feature described under "Content (folder exclusion, OMC-040)" above lets you remove entire folders from every tool's reach, but it is a coarse, folder-granularity access control, not content-level filtering — a file that is not excluded is still returned in full, with no scanning of its contents.
...
- **Compromised AI client.** A malicious or compromised MCP client connecting with a valid Bearer token has the same authority as the user, over whatever the vault's current policy exposes. The Bearer token is still the trust boundary that decides *whether* a client can talk to this server at all; rotate it (Settings → Access Control → Regenerate) if you suspect compromise. **Amended (OMC-040):** the token is no longer the *only* boundary that shapes what an authenticated client can reach — the folder-exclusion policy also narrows it, vault-wide today, per-token in a later phase (ADR-0020). Neither narrows what a client with valid credentials can do *inside* what remains exposed.
...
5. **Vault content review.** Before connecting an MCP client, glance at vault folders containing secrets (`.env` files, credentials notes, exported tokens) and consider whether the AI client's authority should extend to those folders. **Amended (OMC-040):** as of the folder-exclusion feature, you can hide entire folders vault-wide from Settings → MCP Connector → Hidden folders — but it is a coarse, best-effort guardrail (see "Content (folder exclusion, OMC-040)" above), not a hard per-token ACL yet. Do not rely on it alone for content you would not want any connected client to see under any circumstance.
