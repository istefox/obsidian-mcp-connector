...
## Amendment: inline module and browser CSP regression (#538)

The search view failed to initialize in Pi MCP Adapter 2.34.0 because its loader imported the embedded SDK through a Blob URL while the host served `script-src 'self' 'unsafe-inline'`. Chrome reproduced the blocked import and the CSP violation. The Bun DOM tests do not enforce a browser CSP, so they could not detect this failure.

The view entry at `assets/mcp-apps/searchResults.js` statically imports the SDK's public exports. `scripts/mcp-apps/generateAppHtml.ts` compiles the view and SDK into one self-contained ESM bundle and inserts it into the HTML shell as an inline module. This supersedes D10's direct insertion of the SDK bundle and the runtime Blob loader: there is no runtime module fetch, no additional CSP permission and no `unsafe-eval`. Hosts must still allow the inline module. The existing closing-script check and generated-resource drift test remain in place. The required CI job pins Bun 1.3.10 because the generated bytes now depend on the bundler as well as the entry, shell and installed SDK.

`bun run test:mcp-app-browser` serves the generated HTML in an iframe under the restrictive policy and verifies initialization, rows, empty/error states, cancellation, theme changes and the open-link request. The regression fails with the original resource and passes with the combined inline resource. This synthetic host test does not certify an external application opening or every client's sandbox. Separate local testing exercised Pi's UI server, bridge and sandbox; WKWebView/Glimpse was not tested.

Install the test browser with `bunx playwright install chromium`; alternatively run `PLAYWRIGHT_CHANNEL=chrome bun run test:mcp-app-browser` with Chrome installed. CI installs Chromium and runs the test inside `check-and-test`. The historical size measurements above remain historical: the generated header records the compiled module's actual byte counts and compiler version instead of repeating the old production-build figures.
